...
LOCAL_INLINE FUNC(void, ADC_CODE) Adc_ReportDetRuntimeError
(
    VAR(uint8, AUTOMATIC) u8ServiceId,
    VAR(uint8, AUTOMATIC) u8ErrorId
);
...
*
* @return     void
* @implements  Adc_ReportDetRuntimeError_Activity
*/
LOCAL_INLINE FUNC(void, ADC_CODE) Adc_ReportDetRuntimeError
(
    VAR(uint8, AUTOMATIC) u8ServiceId,
...
)
{
    ...
    /* ADC458, ADC457, CPR-MCAL-718.adc */
    if (NULL_PTR == pPtrVal)
    {
        ValidPtr = (Std_ReturnType)E_NOT_OK;
...
)
{
    ...
    /* CPR-MCAL-792.adc, CPR-MCAL-793.adc, CPR-MCAL-794.adc, PR-MCAL-3186.adc */
...
)
{
    ...
    if (NULL_PTR == Adc_pCfgPtr)
    {
        /* ADC300, ADC301, ADC302, ADC299, ADC298, ADC297, ADC296, ADC295, ADC294, ADC434 */
        Adc_ReportDetError(u8ServiceId, (uint8)ADC_E_UNINIT);
    }
    else if (Group >= Adc_pCfgPtr->GroupCount)
    {
        /* ADC218, ADC225, ADC131, ADC130, ADC129, ADC128, ADC152, ADC126, ADC125, ADC423 */
...
)
{
    ...
    /* Find the right place in Queue (ADC332, ADC417) */
    /**
     * @violates @ref Adc_c_REF_4 Array indexing shall be the only allowed form of pointer arithmetic
     */
...
)
{
    ...
    switch(u8ServiceId)
    {
        case ADC_ENABLECTUCONTROLMODE_ID:
        {
            ...
            {
                ValidState = (Std_ReturnType)E_NOT_OK;
                Adc_ReportDetRuntimeError((uint8)ADC_ENABLECTUCONTROLMODE_ID, (uint8)ADC_E_BUSY);
            }
            break;
        }
...
(
    VAR(Adc_GroupType, AUTOMATIC) Group,
    CONSTP2VAR(Adc_ValueGroupType, AUTOMATIC, ADC_APPL_DATA) DataBufferPtr
)
{
    /* ADC419 */
...
{
    /* ADC367, ADC356, ADC156, ADC061, ADC413 */
...
{
    /* ADC370, ADC114, ADC114, ADC413, ADC357 */
    /* ADC Hardware unit on which the requested group will run */
    VAR(Adc_HwUnitType, AUTOMATIC) Unit = 0U;
        ...
        if((Std_ReturnType)E_OK == Adc_ValidatePowerStatePrepare(ADC_SETPOWERSTATE_ID, Result))
        {
            if((Std_ReturnType)E_OK == Adc_ValidatePowerStateSupport(ADC_SETPOWERSTATE_ID, Adc_eTargetState, Result))
            {
                if((Std_ReturnType)E_OK == Adc_ValidateIdleState(ADC_SETPOWERSTATE_ID, Result))
                {
                    ...
                    {
                        /* ADC482 (ADC482) :The target state is the same as the current one, return a E_OK and no action is executed  */
                    ...
                    else
                    {
                        ...
                        {
                            #if (ADC_DEV_ERROR_DETECT == STD_ON)
                            Adc_ReportDetRuntimeError(ADC_SETPOWERSTATE_ID, ADC_E_TRANSITION_NOT_POSSIBLE);
                            #endif /* (ADC_DEV_ERROR_DETECT == STD_ON) */
                            /* Hardware failure */
...
{
    /* SMCAL_SW121.adc, PR-MCAL-3012.adc */
#if (ADC_VALIDATE_CALL_AND_UNIT == STD_ON)
