...
/** @violates @ref Adc_Pdb_Irq_c_REF_4 MISRA-C:2004 Rule 1.4: Identifier clash.*/
#define ADC_AR_RELEASE_MINOR_VERSION_PDB_IRQ_C     3
/** @violates @ref Adc_Pdb_Irq_c_REF_4 MISRA-C:2004 Rule 1.4: Identifier clash.*/
#define ADC_AR_RELEASE_REVISION_VERSION_PDB_IRQ_C  1
...
)
{
    ...
    VAR(uint32, AUTOMATIC) u32TempSC = 0UL;
    VAR(uint8, AUTOMATIC) u8PDBChannelCount;
    VAR(boolean, AUTOMATIC) bErrorFlag = (boolean)FALSE;

    /* Check if any of PDB channels have sequence error */
    for (u8PDBChannelCount = 0U; u8PDBChannelCount < ADC_PDB_CHANNELS_U8; u8PDBChannelCount++)
    {
        ...
        if(REG_READ32(PDB_CHnS_REG_ADDR32(u8Unit, u8PDBChannelCount)) & PDB_ERR_FLAGS_MASK_U32)
        {
            bErrorFlag = (boolean)TRUE;
            
            /* Save the value from SC register */
            /** @violates @ref Adc_Pdb_Irq_c_REF_3 cast from unsigned long to pointer */
            /** @violates @ref Adc_Pdb_Irq_c_REF_5 A cast should not be performed between a pointer type and an integral type. */
            ...
            /* Clear error flags of the PDB channel */
            /** @violates @ref Adc_Pdb_Irq_c_REF_3 cast from unsigned long to pointer */
            /** @violates @ref Adc_Pdb_Irq_c_REF_5 A cast should not be performed between a pointer type and an integral type. */
            REG_WRITE32(PDB_CHnS_REG_ADDR32(u8Unit, u8PDBChannelCount), PDB_ERR_FLAGS_CLEAR_U32);
    ...
    if((boolean)TRUE == bErrorFlag)
    {
            if(NULL_PTR != Adc_pCfgPtr)
            {
                    ...
                    {
                        Adc_Pdb_ReConfigureDma(u8Unit, u32Timeout);
                    }
                  #endif /* (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF) */
                #endif /* ADC_DMA_SUPPORTED */
...
)
{
    ...
    VAR(uint32, AUTOMATIC) u32ScValue;
    ...
    if((uint8)STD_ON == Adc_pCfgPtr->pGroups[Group].u8AdcGroupChannelDelays)
    {
        for(u8ScRegister = 0U; u8ScRegister < Adc_aUnitStatus[u8Unit].u8Sc1Used; u8ScRegister++)
        {
            /** @violates @ref Adc_Pdb_Irq_c_REF_3 cast from unsigned long to pointer */
            /** @violates @ref Adc_Pdb_Irq_c_REF_5 A cast should not be performed between a pointer type and an integral type. */
            u32ScValue = REG_READ32(ADC12BSARV2_SC1_REG_ADDR32(u8Unit, (uint32)u8ScRegister));
            /* Aborts the current conversion */
            /** @violates @ref Adc_Pdb_Irq_c_REF_3 cast from unsigned long to pointer */
            /** @violates @ref Adc_Pdb_Irq_c_REF_5 A cast should not be performed between a pointer type and an integral type. */
            REG_WRITE32(ADC12BSARV2_SC1_REG_ADDR32(u8Unit, (uint32)u8ScRegister), u32ScValue);
            /** @violates @ref Adc_Pdb_Irq_c_REF_3 cast from unsigned long to pointer */
            /** @violates @ref Adc_Pdb_Irq_c_REF_5 A cast should not be performed between a pointer type and an integral type. */
    ...
    /* ADC235, ADC068 */
    while(((boolean)TRUE == Mcl_DmaIsTransferActive(DmaChannel)) && (u32Timeout > (uint32)0U))
    {
        u32Timeout--;
    }
...
#ifdef ADC_UNIT_0_PDB_ERR_ISR_USED
/** @violates @ref Adc_Pdb_Irq_c_REF_10 External linkage.*/
ISR(Adc_Pdb_ChannelSequenceError0)
{
    VAR(volatile uint32, AUTOMATIC) u32Timeout = 0U;
#ifdef ADC_DMA_SUPPORTED
#if (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF)
    if((uint8)ADC_DMA == Adc_pCfgPtr->Misc.u8Adc_DmaInterruptSoftware[0])
    {
        u32Timeout = ADC_DMA_TRANSFER_TIMEOUT;
    }
#endif /* (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF) */
#endif /* ADC_DMA_SUPPORTED */
    MCAL_FAULT_INJECTION_POINT(ADC_FIP_2_UPDATE_VARIABLE_TIMEOUT);
    Adc_Pdb_ChannelSequenceError(0U, u32Timeout);
}
#endif
...
#ifdef ADC_DMA_SUPPORTED
#if (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF)
    if((uint8)ADC_DMA == Adc_pCfgPtr->Misc.u8Adc_DmaInterruptSoftware[1])
    {
        u32Timeout = ADC_DMA_TRANSFER_TIMEOUT;
    }
#endif /* (ADC_OPTIMIZE_ONESHOT_HW_TRIGGER == STD_OFF) */
#endif /* ADC_DMA_SUPPORTED */
    MCAL_FAULT_INJECTION_POINT(ADC_FIP_3_UPDATE_VARIABLE_TIMEOUT);
    Adc_Pdb_ChannelSequenceError(1U, u32Timeout);
}
#endif
