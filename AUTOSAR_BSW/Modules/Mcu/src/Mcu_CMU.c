...
/** @violates @ref Mcu_CMU_c_REF_5 MISRA 2004 Rule 1.4, The compiler/linker shall be checked to ensure that 31 character
*   significance and case sensitivity are supported for external identifiers. */
#define MCU_CMU_AR_RELEASE_MINOR_VERSION_C       3
/** @violates @ref Mcu_CMU_c_REF_5 MISRA 2004 Rule 1.4, The compiler/linker shall be checked to ensure that 31 character
*   significance and case sensitivity are supported for external identifiers. */
#define MCU_CMU_AR_RELEASE_REVISION_VERSION_C    1
...
CONST(uint32, MCU_CONST) Mcu_CMU_au32BaseAddr[] =
{
#ifdef  CMU0_BASEADDR
...
#if (IPV_CMU != IPV_CMU_00_00_00_01)
/**
* @brief            This function initializes CMU_CSR Register.
* @details          CMU_CSR Register are used for monitoring major frequencis on the platform.
*                       - Mcu_Ipw_InitClock() from IPW.
*
...
*
*/
FUNC(void, MCU_CODE) Mcu_CMU_CSR_RCDIV_Config(P2CONST(Mcu_CMU_ConfigType, AUTOMATIC, MCU_APPL_CONST) pCmu_ConfigPtr)
{
    /* Index for CMU units. */
    VAR(uint8, AUTOMATIC)   u8IndexCmu = 0U;
    ...
    for(u8IndexCmu = 0U; u8IndexCmu < MCU_CMU_UNITS; u8IndexCmu++)
    {
        ...
        /** @violates @ref Mcu_CMU_c_REF_3 Cast from unsigned long to pointer.*/
        /** @violates @ref Mcu_CMU_c_REF_6 The cast is used to access memory mapped registers.*/
        REG_WRITE32(CMU_CSR_X_ADDR32(u8IndexCmu), (pCmu_ConfigPtr-> au32CmuConfig[u8IndexCmu] & CMU_CSR_RCDIV_MASK32));
    }
}
#endif
/**
...
)
{
    ...
    VAR(uint32, AUTOMATIC) u32CmuGcrValue;
    VAR(uint32, AUTOMATIC) u32CmuIerValue;
    ...
    if (NULL_PTR != Mcu_pConfigPtr)
    {
        /** @violates @ref Mcu_CMU_c_REF_3 Cast from unsigned long to pointer.*/
        /** @violates @ref Mcu_CMU_c_REF_6 The cast is used to access memory mapped registers.*/
        u32CmuGcrValue = REG_READ32(CMU_GCR_X_ADDR32(u8CmuIdx));
        
        /** @violates @ref Mcu_CMU_c_REF_3 Cast from unsigned long to pointer.*/
        /** @violates @ref Mcu_CMU_c_REF_6 The cast is used to access memory mapped registers.*/
        u32CmuIerValue = REG_READ32(CMU_IER_X_ADDR32(u8CmuIdx)) & u32IsrMask;
        
        u32CmuIsrValue = u32CmuIsrValue & u32CmuIerValue;

        /* Now check if there is a reason to signal to the upper layer */
        if(CMU_GCR_CMU_ENA_U32 == (u32CmuGcrValue & CMU_GCR_X_FCE_MASK32))
        {
            /*Only checks CMU1*/
            if (
                   ( CMU1_IER_FHH_ENA_U32 == (u32CmuIsrValue & CMU_IER_FHH_MASK32) ) ||
                   ( CMU1_IER_FLL_ENA_U32 == (u32CmuIsrValue & CMU_IER_FLL_MASK32) )
               )
            {
