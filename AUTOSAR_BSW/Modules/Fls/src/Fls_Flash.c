...
static FUNC( Fls_LLDReturnType, FLS_CODE ) Fls_Flash_SectorCompareUnalignBytes
(
    P2CONST(volatile uint8, AUTOMATIC, FLS_APPL_CONST ) pReadAddressEndPtr
...
static FUNC( void, FLS_CODE ) Fls_Flash_DisablePrefetchBuff( void );
#endif
...
static FUNC( Fls_LLDReturnType, FLS_CODE ) Fls_Flash_WaitForOperationFinish( void )
{
    ...
    * and an integral type
    */
    MCAL_FAULT_INJECTION_POINT(FLS_FIP_1_WAIT_FOR_OPERATION_FINISH);
...
* @retval       FLASH_E_PARTITION_ERR operation failed due to invalid configuration or timeout 
*/
static FUNC( Fls_LLDReturnType, FLS_CODE ) Fls_Flash_CheckFlexNvmPartitionRatio(void)
{
    ...
    */    
    ...
    if(
#if (0x8000UL == D_FLASH_SIZE)
       (FLASH_FLEXNVM_DFLASH_EEPROM_32_0_V1 == u32FlexNvmPartSize) || (FLASH_FLEXNVM_DFLASH_EEPROM_32_0_V2 == u32FlexNvmPartSize) ||
#elif (0x10000UL == D_FLASH_SIZE)
       (FLASH_FLEXNVM_DFLASH_EEPROM_64_0_V1 == u32FlexNvmPartSize) || (FLASH_FLEXNVM_DFLASH_EEPROM_64_0_V2 == u32FlexNvmPartSize) ||
#elif (0x80000UL == D_FLASH_SIZE)
       (FLASH_FLEXNVM_DFLASH_EEPROM_512_0_V1 == u32FlexNvmPartSize) ||
#endif
...
*/
/*
* @violates @ref fls_flash_c_REF_21 All declarations and definitions of objects or functions
* at file scope shall have internal linkage unless external linkage required
*/     
...
/* @errata ERR_IPV_FMC_0001 */
            ...
            Mcal_Trusted_Call(Fls_Flash_DisablePrefetchBuff);
        #else
            Fls_Flash_DisablePrefetchBuff();
    ...
    if( FLASH_E_PARTITION_ERR == eFlexNVMConfigurationFlag )   
    {
        /* FlexNVM is not partitioned for only Data Flash */        
            ...
            (void) Det_ReportError( (uint16)FLS_MODULE_ID, FLS_INSTANCE_ID, FLS_INIT_ID, FLS_E_PARTITION_ERR );
        #endif
        ...
        if ((1U == (*(Fls_pConfigPtr->paSectorUnlock))[u32ConfigIter]) && (FLS_CH_INTERN ==(*(Fls_pConfigPtr->paHwCh))[u32ConfigIter])) 
        {
            ...
            */
            if(0U == REG_BIT_GET8(u32SecRegProtAddr, (uint8)(1U << u8SecRegProtBitPos)) ) 
            {
                ...
                #if( FLS_DEV_ERROR_DETECT == STD_ON )
                (void) Det_ReportError( (uint16)FLS_MODULE_ID, FLS_INSTANCE_ID, FLS_INIT_ID, FLS_E_SECTOR_PROTECTED );
                #endif
                    Fls_eLLDJob = FLASH_JOB_NONE;
                    Fls_eLLDJobResult = MEMIF_JOB_FAILED;
                    /*
    ...
    */
    ...
    {
        MCAL_FAULT_INJECTION_POINT(FLS_FIP_3_FLS_ABORT_SUSPENDED);
...
FUNC( Fls_LLDReturnType, FLS_CODE ) Fls_Flash_AbortSuspended( void )
{
    ...
    {
        MCAL_FAULT_INJECTION_POINT(FLS_FIP_2_FLS_ABORT_SUSPENDED);
        ...
        */ 
        ...
        {
            MCAL_FAULT_INJECTION_POINT(FLS_FIP_2_FLS_ABORT_SUSPENDED_1);
            ...
            */ 
            ...
            {
                MCAL_FAULT_INJECTION_POINT(FLS_FIP_2_FLS_ABORT_SUSPENDED_2);
       ...
       )
    {
        ...
        */
        if( 0U != ((uint32)Fls_Flash_pCompareAddressPtr % 4U) )
        {
            ...
            dataCompare  = ((uint32)(*(Fls_Flash_pCompareAddressPtr + 3U)) << 24U);
            /* @violates @ref fls_flash_c_REF_10 Array indexing shall be the only allowed form of pointer arithmetic*/
            dataCompare |= ((uint32)(*(Fls_Flash_pCompareAddressPtr + 2U)) << 16U);
            /* @violates @ref fls_flash_c_REF_10 Array indexing shall be the only allowed form of pointer arithmetic*/
            dataCompare |= ((uint32)(*(Fls_Flash_pCompareAddressPtr + 1U)) << 8U);
            /* @violates @ref fls_flash_c_REF_10 Array indexing shall be the only allowed form of pointer arithmetic*/
            dataCompare |= ((uint32)(*(Fls_Flash_pCompareAddressPtr)));
        } 
        else
        {
            /*
            ...
            * between a pointer to object type and a different pointer to object type
            */
            dataCompare = *((volatile const Fls_Flash_DataBusWidthType*)Fls_Flash_pCompareAddressPtr);
        }
        /*
        * @violates @ref fls_flash_c_REF_2 A cast should not be performed
        * between a pointer to object type and a different pointer to object type
...
FUNC( void, FLS_CODE ) Fls_Flash_MainFunctionWrite( void )
{
    ...
    */
    if(REG_BIT_GET8(FLASH_FSTAT_ADDR32,FLASH_FSTAT_CCIF_U8))
    {
        ...
        * and an integral type
        */
        if((0U == REG_BIT_GET8(FLASH_FSTAT_ADDR32, FLASH_FSTAT_RDCOLERR_U8 | FLASH_FSTAT_ACCERR_U8 | FLASH_FSTAT_FPVIOL_U8 | FLASH_FSTAT_MGSTAT0_U8))
            #if (STD_ON == FLS_SYNCRONIZE_CACHE)
            && ((Std_ReturnType)E_OK == eCacheRetVal)
            #endif                  
          )
...
( CONST(uint32, AUTOMATIC) u32TotalBytes )
{
    ...
    */
    ...
    MCAL_FAULT_INJECTION_POINT(FLS_FIP_4_FLS_FLASH_SECTOR_WRITE);
    ...
    for (u32Counter=0UL; u32Counter < (u32TotalBytes/8U); u32Counter++)
    {
        ...
        * @violates @ref fls_flash_c_REF_8 A cast should not be performed between a pointer type and an integral type
        */
        REG_WRITE8(FLASH_FCCOB4_ADDR32, (uint8)(*(uint8 *)((uint32)Fls_Flash_pSourceAddressPtr + 3U)));
        /* 
        * @violates @ref fls_flash_c_REF_18 Cast from pointer to unsigned long 
        * @violates @ref fls_flash_c_REF_8 A cast should not be performed between a pointer type and an integral type
        */
        REG_WRITE8(FLASH_FCCOB5_ADDR32, (uint8)(*(uint8 *)((uint32)Fls_Flash_pSourceAddressPtr + 2U)));
        /* 
        * @violates @ref fls_flash_c_REF_18 Cast from pointer to unsigned long 
        * @violates @ref fls_flash_c_REF_8 A cast should not be performed between a pointer type and an integral type
        */
        REG_WRITE8(FLASH_FCCOB6_ADDR32, (uint8)(*(uint8 *)((uint32)Fls_Flash_pSourceAddressPtr + 1U)));
        /* 
        * @violates @ref fls_flash_c_REF_18 Cast from pointer to unsigned long 
        * @violates @ref fls_flash_c_REF_8 A cast should not be performed between a pointer type and an integral type
        */
        REG_WRITE8(FLASH_FCCOB7_ADDR32, (uint8)(*(uint8 *)(uint32)Fls_Flash_pSourceAddressPtr));
        /* @violates @ref fls_flash_c_REF_10 Array indexing shall be the only allowed form of pointer arithmetic */
        Fls_Flash_pTargetAddressPtr++;
        /* @violates @ref fls_flash_c_REF_10 Array indexing shall be the only allowed form of pointer arithmetic */
        ...
        * @violates @ref fls_flash_c_REF_8 A cast should not be performed between a pointer type and an integral type
        */
        REG_WRITE8(FLASH_FCCOB8_ADDR32, (uint8)(*(uint8 *)((uint32)Fls_Flash_pSourceAddressPtr + 3U)));
        /* 
        * @violates @ref fls_flash_c_REF_18 Cast from pointer to unsigned long 
        * @violates @ref fls_flash_c_REF_8 A cast should not be performed between a pointer type and an integral type
        */
        REG_WRITE8(FLASH_FCCOB9_ADDR32, (uint8)(*(uint8 *)((uint32)Fls_Flash_pSourceAddressPtr + 2U)));
        /* 
        * @violates @ref fls_flash_c_REF_18 Cast from pointer to unsigned long 
        * @violates @ref fls_flash_c_REF_8 A cast should not be performed between a pointer type and an integral type
        */
        REG_WRITE8(FLASH_FCCOBA_ADDR32, (uint8)(*(uint8 *)((uint32)Fls_Flash_pSourceAddressPtr + 1U)));
        /* 
        * @violates @ref fls_flash_c_REF_18 Cast from pointer to unsigned long 
        * @violates @ref fls_flash_c_REF_8 A cast should not be performed between a pointer type and an integral type
        */
        REG_WRITE8(FLASH_FCCOBB_ADDR32, (uint8)(*(uint8 *)(uint32)Fls_Flash_pSourceAddressPtr));
...
)
{
    ...
    VAR(uint32, AUTOMATIC) Fls_Flash_u32DataCompare32 = 0UL;
...
#else
    eRetVal = Fls_Flash_SectorCompareUnalignBytes(pReadBlockAddressEndPtr);
#endif
#if( FLS_DSI_HANDLER_API == STD_OFF )
   ...
   */
...
#endif
    {
        ...
        */
        ...
        {
            /* @violates @ref fls_flash_c_REF_10 Array indexing shall be the only allowed form of pointer arithmetic*/
            Fls_Flash_u32DataCompare32  = ((uint32)(*(Fls_Flash_pCompareAddressPtr + 3U)) << 24U);
            /* @violates @ref fls_flash_c_REF_10 Array indexing shall be the only allowed form of pointer arithmetic*/
            Fls_Flash_u32DataCompare32 |= ((uint32)(*(Fls_Flash_pCompareAddressPtr + 2U)) << 16U);
            /* @violates @ref fls_flash_c_REF_10 Array indexing shall be the only allowed form of pointer arithmetic*/
            Fls_Flash_u32DataCompare32 |= ((uint32)(*(Fls_Flash_pCompareAddressPtr + 1U)) << 8U);
            /* @violates @ref fls_flash_c_REF_10 Array indexing shall be the only allowed form of pointer arithmetic*/
            Fls_Flash_u32DataCompare32 |= ((uint32)(*(Fls_Flash_pCompareAddressPtr)));
        }
        else
        {
            /*
            ...
            * between a pointer to object type and a different pointer to object type
            */
            Fls_Flash_u32DataCompare32 = *((volatile const Fls_Flash_DataBusWidthType*)Fls_Flash_pCompareAddressPtr);
        }
        /*
        * @violates @ref fls_flash_c_REF_2 A cast should not be performed
        * between a pointer to object type and a different pointer to object type
...
#else
    eRetVal = Fls_Flash_SectorCompareUnalignBytes(pReadAddressEndPtr);
#endif
...
(
#if( FLS_ECC_SYNDROME_AND_DATA_ADDRESS_CHECK == STD_ON )
...
#endif  /*#if( FLS_ECC_SYNDROME_AND_DATA_ADDRESS_CHECK == STD_ON )*/
        /* @violates @ref fls_flash_c_REF_3 Side effects on right hand of logical operator: '||' or '&&' */
      )
    {
        Fls_Flash_eReadStatus = FLASH_E_FAILED;
        u8RetVal = FLS_HANDLED_SKIP;
             ...
             &&
             /* @violates @ref fls_flash_c_REF_3 Side effects on right hand of logical operator: '||' or '&&' */
             (( pExceptionDetailsPtr->data_pt == Fls_Flash_pReadAddressPtr ) || ( pExceptionDetailsPtr->data_pt == Fls_Flash_pCompareAddressPtr ))
#endif  /*#if( FLS_ECC_SYNDROME_AND_DATA_ADDRESS_CHECK == STD_ON )*/
        /* @violates @ref fls_flash_c_REF_3 Side effects on right hand of logical operator: '||' or '&&' */
           )
    {
