...
/* Check if current file and Fls_Qspi header file are of the same Autosar version */
     ...
     (FLS_IPW_AR_RELEASE_REVISION_VERSION_C    != FLS_QSPI_AR_RELEASE_REVISION_VERSION_H))
...
{    
    ...
    { 
        ...
        if(bAsynch)
        {
            eLldRetVal = Fls_Qspi_SectorWriteAsync(u32StartAddr, u32Length, pJobDataSrcPtr);
        }
        else
        {
            eLldRetVal = Fls_Qspi_SectorWriteSync(u32StartAddr, u32Length, pJobDataSrcPtr);
        }
    }
#endif
#endif
