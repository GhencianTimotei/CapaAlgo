...
#define MCL_AR_RELEASE_MINOR_VERSION_C      3
#define MCL_AR_RELEASE_REVISION_VERSION_C   1
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        if ((Std_ReturnType)E_OK == valid)
        {
           ...
           (void)ConfigPtr;
            ...
            Mcl_IPW_Flexio_Init(Mcl_pConfig->pMclFlexioConfig->u32CtrConfig);
#endif
#endif        
#if (MCL_VALIDATE_PARAMS == STD_ON)
        }
...
#if (MCL_ENABLE_FLEXIO == STD_ON)
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_Enable(void)
{
    Mcl_IPW_Flexio_Enable();
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_Disable(void)
{
    Mcl_IPW_Flexio_Disable();
}

/**
 * @brief   This function clears bits from the Shifter Status register of FlexIO
 *
 * @details This function is reentrant and clears bits from the Shifter Status register of FlexIO
 *          each bit from lsb to msb represents a channel from 0 to 7, and a "1" clears the bit 
 *          for the respective channel
 *
 * @param[in]     u8ShifterMask          8bit Mask of the shifter status flags to be cleared
 *
 * @return void
 *
 * @api
 *
 * @pre Mcl_Flexio_Enable must be called before.
 *
 * */
 /** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_ClearShiftStat(VAR(uint8, AUTOMATIC) u8ShifterMask)
{
    Mcl_IPW_Flexio_ClearShiftStat(u8ShifterMask);
}

/**
 * @brief   This function reads bits from the Shifter Status register of FlexIO
 *
 * @details This function is reentrant and reads bits from the Shifter Status register of FlexIO
 *          each bit from lsb to msb represents a channel from 0 to 7, and a "1" clears the bit 
 *          for the respective channel
 *
 * @param[in]     u8ShifterMask          8bit Mask of the shifter status flags to be read
 *
 ...
 * */
 /** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(uint8, MCL_CODE) Mcl_Flexio_ReadShiftStat(VAR(uint8, AUTOMATIC) u8ShifterMask)
{
    VAR(uint8, AUTOMATIC) u8StatValue;

    u8StatValue = Mcl_IPW_Flexio_ReadShiftStat(u8ShifterMask);

    return u8StatValue;
}

/**
 * @brief   This function clears bits from the Shifter Error register of FlexIO
 *
 * @details This function is reentrant and clears bits from the Shifter Error register of FlexIO
 *          each bit from lsb to msb represents a channel from 0 to 7, and a "1" clears the bit 
 *          for the respective channel
 *
 * @param[in]     u8ShifterMask          8bit Mask of the shifter error flags to be cleared
 *
 * @return void
 *
 * @api
 ...
 * */
 /** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_ClearShiftErr(VAR(uint8, AUTOMATIC) u8ShifterMask)
{
    Mcl_IPW_Flexio_ClearShiftErr(u8ShifterMask);
}

/**
 * @brief   This function reads bits from the Shifter Error register of FlexIO
 *
 * @details This function is reentrant and reads bits from the Shifter Error register of FlexIO
 *          each bit from lsb to msb represents a channel from 0 to 7, and a "1" clears the bit 
 *          for the respective channel
 *
 * @param[in]     u8ShifterMask          8bit Mask of the shifter rrror flags to be read
 *
 ...
 * */
 /** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(uint8, MCL_CODE) Mcl_Flexio_ReadShiftErr(VAR(uint8, AUTOMATIC) u8ShifterMask)
{
    VAR(uint8, AUTOMATIC) u8StatValue;

    u8StatValue = Mcl_IPW_Flexio_ReadShiftErr(u8ShifterMask);

    return u8StatValue;
}

/**
 * @brief   This function clears bits from the Timer Status register of FlexIO
 *
 * @details This function is reentrant and clears bits from the Timer Status register of FlexIO
 *          each bit from lsb to msb represents a channel from 0 to 7, and a "1" clears the bit 
 *          for the respective channel
 *
 * @param[in]     u8TimerMask          8bit Mask of the timer status flags to be cleared
 *
 * @return void
 *
 * @api
 ...
 * */
 /** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_ClearTimStat(VAR(uint8, AUTOMATIC) u8TimerMask)
{
    Mcl_IPW_Flexio_ClearTimStat(u8TimerMask);
}

/**
 * @brief   This function reads bits from the Timer Status register of FlexIO
 *
 * @details This function is reentrant and reads bits from the Timer Status register of FlexIO
 *          each bit from lsb to msb represents a channel from 0 to 7, and a "1" reads the bit 
 *          for the respective channel
 *
 * @param[in]     u8TimerMask          8bit Mask of the timer status flags to be cleared
 *
 ...
 * */
 /** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(uint8, MCL_CODE) Mcl_Flexio_ReadTimStat(VAR(uint8, AUTOMATIC) u8TimerMask)
{
    VAR(uint8, AUTOMATIC) u8StatValue;

    u8StatValue = Mcl_IPW_Flexio_ReadTimStat(u8TimerMask);

    return u8StatValue;
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_WriteShiftSien(VAR(uint8, AUTOMATIC) u8ShifterMask, VAR(uint8, AUTOMATIC) u8ShifterEnableMask)
{
    Mcl_IPW_Flexio_WriteShiftSien(u8ShifterMask, u8ShifterEnableMask);
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(uint8, MCL_CODE) Mcl_Flexio_ReadShiftSien(VAR(uint8, AUTOMATIC) u8ShifterMask)
{
    VAR(uint8, AUTOMATIC) u8StatValue;

    u8StatValue = Mcl_IPW_Flexio_ReadShiftSien(u8ShifterMask);

    return u8StatValue;
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_WriteShiftEien(VAR(uint8, AUTOMATIC) u8ShifterMask, VAR(uint8, AUTOMATIC) u8ShifterEnableMask)
{
    Mcl_IPW_Flexio_WriteShiftEien(u8ShifterMask, u8ShifterEnableMask);
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(uint8, MCL_CODE) Mcl_Flexio_ReadShiftEien(VAR(uint8, AUTOMATIC) u8ShifterMask)
{
    VAR(uint8, AUTOMATIC) u8StatValue;

    u8StatValue = Mcl_IPW_Flexio_ReadShiftEien(u8ShifterMask);

    return u8StatValue;
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_WriteTimIen(VAR(uint8, AUTOMATIC) u8TimerMask, VAR(uint8, AUTOMATIC) u8TimerEnableMask)
{
    Mcl_IPW_Flexio_WriteTimIen(u8TimerMask, u8TimerEnableMask);
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(uint8, MCL_CODE) Mcl_Flexio_ReadTimIen(VAR(uint8, AUTOMATIC) u8TimerMask)
{
    VAR(uint8, AUTOMATIC) u8StatValue;

    u8StatValue = Mcl_IPW_Flexio_ReadTimIen(u8TimerMask);

    return u8StatValue;
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_WriteShiftSden(VAR(uint8, AUTOMATIC) u8ShifterMask, VAR(uint8, AUTOMATIC) u8ShifterEnableMask)
{
    Mcl_IPW_Flexio_WriteShiftSden(u8ShifterMask, u8ShifterEnableMask);
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(uint8, MCL_CODE) Mcl_Flexio_ReadShiftSden(VAR(uint8, AUTOMATIC) u8ShifterMask)
{
    VAR(uint8, AUTOMATIC) u8StatValue;

    u8StatValue = Mcl_IPW_Flexio_ReadShiftSden(u8ShifterMask);

    return u8StatValue;
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_EnableInterrupts(VAR(uint8, AUTOMATIC) u8ShifterMask, VAR(uint8, AUTOMATIC) u8ErrMask, VAR(uint8, AUTOMATIC) u8TimerMask)
{
    Mcl_IPW_Flexio_EnableInterrupts(u8ShifterMask, u8ErrMask, u8TimerMask);
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_DisableInterrupts(VAR(uint8, AUTOMATIC) u8ShifterMask, VAR(uint8, AUTOMATIC) u8ErrMask, VAR(uint8, AUTOMATIC) u8TimerMask)
{
    Mcl_IPW_Flexio_DisableInterrupts(u8ShifterMask, u8ErrMask, u8TimerMask);
}
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_Flexio_SoftwareReset(void)
{
    Mcl_IPW_Flexio_SoftwareReset();
}
...
* @param[in]   u16ElementSyncList      Ftm module mask value
*
* @return      void
*
*/
/** @violates @ref Mcl_c_5 Violates MISRA 2004 Required Rule 8.10 could be made static */
FUNC(void, MCL_CODE) Mcl_SelectCommonTimebase (VAR (uint8, AUTOMATIC) ModuleId, VAR (uint16, AUTOMATIC) u16ElementSyncList)
{

#if (MCL_VALIDATE_GLOBAL_CALL == STD_ON)
    VAR (Std_ReturnType, AUTOMATIC) CallIsValid;

    CallIsValid = Mcl_ValidateGlobalCall(MCL_SELECTCOMMONTIMEBASE_ID);

    if ((Std_ReturnType)E_OK == CallIsValid)
    {
#endif
        Mcl_Ipw_SelectCommonTimebase(ModuleId, u16ElementSyncList);
